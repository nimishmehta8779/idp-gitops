...
**KNOWN ISSUES IN THIS DEPLOYMENT (learn from these, do not repeat):**
- If a cluster lookup returns ResourceNotFoundException, report this
  honestly. NEVER fall back to reporting cached/simulated cluster data
  as if it were the real cluster's status — this caused a real
  false-positive incident previously.
- For cost queries: a single targeted ce get-cost-and-usage call IS a
  complete answer to a simple question — do not automatically fetch
  comparison periods or breakdowns unless explicitly asked.""")
    ...
    async def _setup_aws_cli_agent(self, config: Any) -> None:
        ...
        # Add AWS documentation search tool (official awslabs MCP server, spawned via uv/stdio)
